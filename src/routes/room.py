...
async def join_room(room_code:str, db=Depends(get_db), user= Depends(is_authenticated), redis=Depends(get_redis)):
    result= await room_service.join_room(room_code, db, user, redis)
...
async def get_room_details(room_code:str, user=Depends(is_authenticated), redis=Depends(get_redis)):
    return await room_service.fetch_room_details(room_code, user.id, redis)
...
async def leave_room(room_code:str, db=Depends(get_db), user=Depends(is_authenticated), redis=Depends(get_redis)):
    return await room_service.leave_room(room_code, user.id, db, redis)
